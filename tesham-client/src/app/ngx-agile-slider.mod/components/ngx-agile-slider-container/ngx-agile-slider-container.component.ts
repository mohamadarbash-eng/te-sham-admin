...
/**
 * This components represents a carousel.pck container which contains the main functionality.
 */
@Component({
  selector: 'te-app-ngx-agile-slider-container',
  styleUrls: ['ngx-agile-slider-container.component.scss'],
...
export class SliderContainerComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(private _lazyLoaderService: LazyLoaderService, private _renderer: Renderer2, private detectChangeRef: ChangeDetectorRef) {
  }
  ...
  public ngAfterViewInit() {
    ...
    this.initializationTrigger.emit(true);
    this.initSlider();
  ...
  /**
   * Initializes the carousel.pck and fires the events.
   */
  public initSlider() {
  ...
  public prepareAutoSwitchOff() {
    this._resizeSubscription = fromEvent(window, 'resize').pipe(debounceTime(250))
      .subscribe(() => {
        this.buildContainer();
      });
  }
  ...
  public translate(duration: number = this._concatenatedSliderOptions.duration, offset?: number) {
    ...
    if (this._browserPrefixes.translate3dAvailable) {
      ...
      this.sliderList
        .nativeElement
        .style[this._browserPrefixes.transformPrefix] = 'pipe(' + translateValue + 'px, 0, 0)';
    }
  ...
  public setLoadedImage(src: string, index: number, node: any, nodeIndex = 0) {
    _.forEach(node.children, (child: any) => {
      if (child.nodeName === 'IMG' && index === nodeIndex) {
        this._renderer.setAttribute(child, 'src', src);
      } else {
  ...
  public getBrowserSpecificPrefixes(): BrowserSpecificPrefixesInterface {
    ...
    translate3dAvailable = !!window.getComputedStyle(this.container.nativeElement).getPropertyValue(transformPrefix);
